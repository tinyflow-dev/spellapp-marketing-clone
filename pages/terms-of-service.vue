<script setup>
const { data } = await useAsyncData(() => queryCollection("pages").where("stem", "=", "pages/terms-of-service").first());
const { activeHeadingId } = useActiveTOCLink();
const { formatDate } = useFormattedDate();
const formattedDate = formatDate(data.value?.meta?.date);

usePageMeta({
    title: "Terms of Service | Spell",
    description: "Read our terms of service to understand how Spell works and what you can and cannot do with it.",
});
</script>
...
<template>
    <header class="section-legal-header u-header-spacing-top u-spacing-block u-text-center u-position-relative u-stacking-context">
        ...
        <div class="container u-text-center">
            <h1 class="u-heading-h1">{{ data?.title || "Terms of Service" }}</h1>
